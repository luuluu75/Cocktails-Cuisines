  ...
  function grabInput(event) {
    ...
    if ($(this).attr("type") === "meal") {
      ...
      var recipeInstruction = $(
        `<div><h3>Instructions</h3> ${result.strInstructions} </div> <br>`
      );
      ...
      if (result.strYoutube != null) {
        var recipeVideo = $(
          `<div><h3> Video</h3><iframe width="420" height="315" value=${result.strYoutube.slice(
            -11
          )} src= "https://www.youtube.com/embed/${result.strYoutube.slice(
            -11
      ...
      var recipeIngredient = `<h3>Ingredients</h3><li>  ${ingredients
      ...
      success: function (response) {
        ...
        for (var i = 0; i < 5; i++) {
          if (response.meals[i] !== undefined) {
            var accordionMealTest = `<div class="grid-container" id="mainContent">
      ...
      success: function (response) {
        ...
        for (var i = 0; i < 5; i++) {
          if (response.drinks[i] != undefined) {
            var accordionDrinkTest = `<div class="grid-container" id="mainContent">
